...
每个数据集采用五折交叉拟合。每一折只允许设计查询参与区域划分，测试查询不能影响共访问边、区域边界、区域特征、探测区域和预算选择。
...
> 使用自然语言查询产生的文档共访问区域作为候选物化单元，通过少量真实 GraphRAG 探测学习区域级质量收益，再在全局构建预算下选择区域图。
...
5. 系统预测未探测区域的收益，并在全局预算下联合需求、收益和成本选择区域。
